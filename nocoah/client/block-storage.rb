    ...
    module Client
        ...
        class BlockStorage < Base
            ...
            def get_volume_type_list
                json_data = api_get( "/#{@identity.tenant_id}/types", error_message: "Failed to get volume type list." )
            ...
            def get_volume_type_item( volume_type_id )
                json_data = api_get( "/#{@identity.tenant_id}/types/#{volume_type_id}", error_message: "Failed to get volume type item (volume_type_id: #{volume_type_id})." )
            ...
            def get_volume_detail_list
                json_data = api_get( "/#{@identity.tenant_id}/volumes", error_message: "Failed to get volume list." )
                return [] unless json_data.key?( 'volumes' )
            ...
            def get_volume_detail( volume_id )
                json_data = api_get( "/#{@identity.tenant_id}/volumes/detail", error_message: "Failed to get volume detail list." )
            ...
            def create_volume( volume_id: nil, name: nil, size:, **options )
                ...
                options_2[:source_volid] = volume_id unless volume_id.nil?
                options_2[:name] = name unless name.nil?
                options_2[:size] = size
                body = {
                    volume: options_2
                }

                json_data = api_post( "/#{@identity.tenant_id}/volumes", body: body, error_message: "Failed to create volume (volume_id: #{volume_id}, name: #{name}, size: #{size}, options: #{options})." )
                return nil unless json_data.key?( 'volume' )
            ...
            def delete_volume( volume_id )
                api_delete( "/#{@identity.tenant_id}/volumes/#{volume_id}", error_message: "Failed to delete volume (volume_id: #{volume_id})." ) do | res |
                    volume_id
                end
            end
            ...
            def save_volume_to_image( volume_id, image_name:, **options )
                json_data = api_get( "/#{@identity.tenant_id}/volumes/#{volume_id}", error_message: "Failed to save volume (volume_id: #{volume_id}, image_name: #{image_name}, options: #{options_org})." )
            end
